...
/**
 * 
 * <p>Title: IDCardUtil.java</p>    
 * <p>Description: 身份证相关帮助类</p>     
...
public class IDCardUtil {
	 ...
	 * @author yuanxing
	 * @version 1.0
	 * @param Ai 18位身份证的前17位
	 * @return 新身份证的最后一位
	 */
	private static String getVerifyCode(String Ai) {
		int TotalmulAiWi = 0;
		for (int i = 0; i < 17; i++) {
...
//    System.out.println("转换后的身份证号码为："+update2eighteen("330521820721052"));;
		//String code = "510107740603167";
		//code = Lower2Fifteen(code);
		String code = "510107740603167";
		code = update2Eighteen(code);
...
/*********************************** 身份证验证结束 ****************************************/
	 ...
	 * @throws ParseException
	 */
	public static String update2Eighteen(String code) throws ParseException {
		if (StringUtils.isEmpty(code)) {
			return "";
		}
		code = code.trim();
		if (code.length() != 15 || !"".equals(IDCardValidate(code))) {
			return "";
		}
		code = code.substring(0, 6) + "19" + code.substring(6);
		code = code + getVerifyCode(code);
		return code;
	}

	/**
	 * 将18位身份证号码还原为15位身份证号码
	 *
	 * @author yuanxing
	 * @version 1.0
	 * @param code 18位身份证号码
	 ...
	 * @throws ParseException
	 */
	public static String Lower2Fifteen(String code) throws ParseException {
		if (StringUtils.isEmpty(code)) {
			return "";
		}
		code = code.trim();
		if (code.length() != 18 || !"".equals(IDCardValidate(code))) {
			return "";
		}
		code = code.substring(0, 6) + code.substring(8,17);
		return code;
	}
}
